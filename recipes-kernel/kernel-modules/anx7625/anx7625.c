...
#undef DRM_DEV_ERROR
#endif
//#define DRM_DEV_ERROR(dev, fmt, ...) dev_printk(KERN_DEBUG, dev, fmt, ##__VA_ARGS__)
#define DRM_DEV_ERROR(dev, fmt, ...) printk("%s"fmt, KERN_ERR, ##__VA_ARGS__)

#ifdef DRM_ERROR
#undef DRM_ERROR
#endif
#define DRM_ERROR(fmt, ...) printk("%s"fmt, KERN_ERR, ##__VA_ARGS__)

#ifdef DRM_DEV_DEBUG_DRIVER
#undef DRM_DEV_DEBUG_DRIVER
#endif
//#define DRM_DEV_DEBUG_DRIVER(dev, fmt, ...) dev_printk(KERN_DEBUG, dev, fmt, ##__VA_ARGS__)
#define DRM_DEV_DEBUG_DRIVER(dev, fmt, ...) printk("%s"fmt, KERN_ERR, ##__VA_ARGS__)
...
static void anx7625_init_gpio(struct anx7625_data *platform)
{
	...
	DRM_DEV_DEBUG_DRIVER(dev, "anx7625 init gpio, dev %p\n", dev);
	...
	DRM_DEV_DEBUG_DRIVER(dev, "anx7625 init gpio, enable %p, reset %p\n",
						 platform->pdata.gpio_p_on, platform->pdata.gpio_reset);
			    ...
			    struct anx7625_platform_data *pdata)
{
	...
	pdata->node.panel_node = of_graph_get_port_by_id(np, 3);
			     ...
			     const struct i2c_device_id *id)
{
	...
	int ret = 0;
	struct device *dev = &client->dev;
DRM_DEV_DEBUG_DRIVER(dev, "anx7625_i2c_probe\n");
...
DRM_DEV_DEBUG_DRIVER(dev, "anx7625_i2c_probe anx7625_parse_dt\n");
