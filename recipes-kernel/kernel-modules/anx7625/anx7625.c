...
static int anx7625_video_mute_control(struct anx7625_data *ctx, u8 status)
{
  int ret;
...
static int anx7625_odfc_config(struct anx7625_data *ctx, u8 post_divider)
{
  int ret;
...

{
  ...
  ret = anx7625_reg_write(ctx, ctx->i2c.rx_p1_client, MIPI_PHY_CONTROL_3, val);
  ...
  /* enable DSI mode*/
  ret |= anx7625_write_or(ctx, ctx->i2c.rx_p1_client,
                          MIPI_DIGITAL_PLL_18, SELECT_DSI << MIPI_DPI_SELECT);
...
printk("is_anx_dongle return 0\n");
    return 0;
  }
printk("is_anx_dongle return -ENXIO\n");
  return -ENXIO;
}
  ...
  if (!is_anx_dongle(ctx)) {
    ...
    if (data_buf[0] == 0x37 && data_buf[1] == 0x37 &&
      ...
      *bw = 0x19;
       ...
       */
      if (((*bw) == 0) || ((*bw) == 0xff)) {
        sp_tx_aux_dpcdread_bytes(ctx, 0x00, 0x00, DPCD_MAX_LINK_RATE, 1, bw);
      }
    }
  } else {
    sp_tx_aux_dpcdread_bytes(ctx, 0x00, 0x00, DPCD_MAX_LINK_RATE, 1, bw);
...
static int sp_tx_rst_aux(struct anx7625_data *ctx)
{
  ...
  ret = anx7625_write_or(ctx, ctx->i2c.tx_p2_client, RST_CTRL2, AUX_RST);
  ret |= anx7625_write_and(ctx, ctx->i2c.tx_p2_client, RST_CTRL2, ~AUX_RST);
  return ret;
}
...
static int sp_tx_edid_read(struct anx7625_data *ctx, u8 *pedid_blocks_buf)
{
  ...
  ret |= anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, AP_AUX_ADDR_15_8, 0);
  ret |= anx7625_write_and(ctx, ctx->i2c.rx_p0_client, AP_AUX_ADDR_19_16, 0xf0);
  if (ret < 0) {
  ...
  do {
    ...
    case 3:
      ...
      for (j = 0; j < 8; j++) {
        ...
        segments_edid_read(ctx, count / 2, pblock_buf, offset);
...
static void anx7625_drp_toggle_enable(struct anx7625_data *ctx)
{
  ...
  /* config toggle */
  ret |= anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_ROLE_CONTROL, 0x45);
  ret |= anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_COMMAND, 0x99);
  ret |= anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_ANALOG_CTRL_1, 0xA0);
  ret |= anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_ANALOG_CTRL_1, 0xE0);
...
static int anx7625_ocm_loading_check(struct anx7625_data *ctx)
{
  ...
  DRM_DEV_DEBUG_DRIVER(dev, "Firmware ver %02x%02x,",
        anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, OCM_FW_VERSION),
        anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, OCM_FW_REVERSION));
...
static void anx7625_power_on_init(struct anx7625_data *ctx)
{
  ...
  printk("anx7625_power_on_init failed\n");
}
...
static void anx7625_start_dp_work(struct anx7625_data *ctx)
{
  ...
  DRM_DEV_DEBUG_DRIVER(dev, "MAX BW=%02x, MAX Lane cnt=%02x, HDCP=%02x\n",
                       ...
                       (u32)hdcp_cap);
//Name: MIPI_PPS_REG_6  I2C Address: 0x84  Offset: 0x86
...
static void anx7625_hpd_polling(struct anx7625_data *ctx)
{
  ...
  if (atomic_read(&ctx->power_status) != 1) {
    DRM_DEV_DEBUG_DRIVER(dev, "No need to polling HPD status.\n");
    return;
  }
  ...
  if (ret) {
    ...
    anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_INTR_ALERT_1, 0xFF);
    anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, GPIO_3_MAP, 0);
  }
...
static void anx7625_low_power_mode_check(struct anx7625_data *ctx, int state)
{
  struct device *dev = &ctx->client->dev;
...
void handle_intr_vector(struct anx7625_data *ctx)
{
  unsigned char sys_status;
  u8 intr_vector;
  u8 status;

  intr_vector = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_3_MAP);

  printk("handle_intr_vector: intr vector = 0x%x\n",  intr_vector);

  //WriteReg(OCM_SLAVE_I2C_ADDR, INTERFACE_CHANGE_INT,
  //  intr_vector & (~intr_vector));
  anx7625_reg_write(ctx, ctx->i2c.rx_p0_client, GPIO_3_MAP,
    intr_vector & (~intr_vector));

  if ((~INTR_MASK_SETTING) & intr_vector & CC_STATUS_CHANGE) {
    //status = ReadReg(OCM_SLAVE_I2C_ADDR, NEW_CC_STATUS);
    status = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, NEW_CC_STATUS);
    //pd_cc_status_default_func(status);
    printk("handle_intr_vector: cc status %x\n", status);
  }
  sys_status = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_4_MAP);
  printk("handle_intr_vector: system status = 0x%x\n",  sys_status);

  if ((~INTR_MASK_SETTING) &
    ((intr_vector & VBUS_CHANGE) | STS_VBUS_CHANGE)) {
    status = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_4_MAP);
    //pd_vbus_control_default_func(status & VBUS_STATUS);
    printk("handle_intr_vector: VBUS_CHANGE 0x%x\n", status & VBUS_STATUS);
  }
  if ((~INTR_MASK_SETTING) &
    ((intr_vector & VCONN_CHANGE) | STS_VCONN_CHANGE)) {
    status = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_4_MAP);
    //pd_vconn_control_default_func(status & VCONN_STATUS);
    printk("handle_intr_vector: VCONN_CHANGE 0x%x\n", status & VCONN_STATUS);
  }
  if ((~INTR_MASK_SETTING) &
    ((intr_vector & DATA_ROLE_CHANGE) | STS_DATA_ROLE_CHANGE)) {
    status = anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_4_MAP);
    //pd_drole_change_default_func(status & DATA_ROLE);
    printk("handle_intr_vector: DATA_ROLE_CHANGE 0x%x\n", status & DATA_ROLE);
  }
  ...
  on = status & HPD_STATUS;
//void dp_hpd_change_default_func(bool on)
//{
  /* hpd changed */
  printk("dp_hpd_change_default_func: %d\n", (unsigned int)on);

  if (on == 0) {
    printk("dp_hpd_change_default_func: HPD low\n");
    anx7625_stop_dp_work(ctx);
  } else {
    printk("dp_hpd_change_default_func: HPD high\n");
    //anx7625_start_dp();
    anx7625_start_dp_work(ctx);
  }
//}
    printk("handle_intr_vector: HPD_STATUS_CHANGE 0x%x\n", status & HPD_STATUS);
  }
...
static int anx7625_handle_intr_comm(struct anx7625_data *ctx)
{
  ...
  intr_type = anx7625_reg_read(ctx, ctx->i2c.tcpc_client, TCPC_INTR_ALERT_1);
  ...
  if (intr_vector & TCPC_INTR_SOFTWARE_INT) {
    DRM_DEV_DEBUG_DRIVER(dev, "Received software interrupt\n");
    handle_intr_vector(ctx);
  }
  if (intr_vector & TCPC_INTR_RECEIVED_MSG) {
    DRM_DEV_DEBUG_DRIVER(dev, "Received interface message\n");
  }
  while (anx7625_reg_read(ctx, ctx->i2c.rx_p0_client, GPIO_3_MAP) != 0){
    handle_intr_vector(ctx);
  }
  anx7625_reg_write(ctx, ctx->i2c.tcpc_client, TCPC_INTR_ALERT_1, 0xFF);
...
static int anx7625_hpd_change_detect(struct anx7625_data *ctx)
{
  ...
  DRM_DEV_DEBUG_DRIVER(dev, "Entry, power_status=%d\n",
          (u32)atomic_read(&ctx->power_status));

  if (atomic_read(&ctx->auto_start) == 1) {
    DRM_DEV_DEBUG_DRIVER(dev, "auto_start=1\n");
  ...
  if (atomic_read(&ctx->power_status) == 0) {
    DRM_DEV_DEBUG_DRIVER(dev, "power_status=0\n");
    if (atomic_read(&ctx->cable_connected) == 1) {
      DRM_DEV_DEBUG_DRIVER(dev, "cable_connected=1\n");
//      atomic_set(&ctx->power_status, 1);
    ...
    if (atomic_read(&ctx->cable_connected) == 0) {
      DRM_DEV_DEBUG_DRIVER(dev, "cable_connected=0\n");
  ...
  if (atomic_read(&ctx->power_status) == 1) {
    DRM_DEV_DEBUG_DRIVER(dev, "power_status=1\n");
  ...
  if (atomic_read(&ctx->alert_arrived) == 1) {
    DRM_DEV_DEBUG_DRIVER(dev, "alert_arrived=1\n");
...
{
  int event;
  struct anx7625_data *ctx = container_of(work, struct anx7625_data, work.work);
                            ...
                            struct anx7625_platform_data *pdata)
{
  ...
  of_property_read_u32(dev->of_node, "panel_flags", &pdata->panel_flags);
  ...
  if (pdata->mipi_dsi_host_node) {
    ...
    pdata->mipi_dpi_host_node = of_graph_get_remote_node(np, 1, 0);
                                                ...
                                                bool force)
{
  ...
  DRM_DEV_DEBUG_DRIVER(dev, "drm detect, force %d\n", force);
                ...
                struct i2c_client *client)
{
  ...
  ctx->i2c.tx_p1_client = i2c_new_dummy(client->adapter, TX_P1_ADDR >> 1);
  if (!ctx->i2c.tx_p1_client)
    goto free_tx_p0;

  ctx->i2c.tx_p2_client = i2c_new_dummy(client->adapter, TX_P2_ADDR >> 1);
  if (!ctx->i2c.tx_p2_client)
    goto free_tx_p1;

  ctx->i2c.rx_p0_client = i2c_new_dummy(client->adapter, RX_P0_ADDR >> 1);
  if (!ctx->i2c.rx_p0_client)
    goto free_tx_p2;

  ctx->i2c.rx_p1_client = i2c_new_dummy(client->adapter, RX_P1_ADDR >> 1);
  if (!ctx->i2c.rx_p1_client)
    goto free_rx_p0;

  ctx->i2c.rx_p2_client = i2c_new_dummy(client->adapter, RX_P2_ADDR >> 1);
  if (!ctx->i2c.rx_p2_client)
    goto free_rx_p1;

  ctx->i2c.tcpc_client = i2c_new_dummy(client->adapter, TCPC_INTERFACE_ADDR >> 1);
                                      ...
                                      const char *buf, size_t count)
{
  ...
  for (i = 0; i < sizeof(addr); i++) {
    ...
    anx7625_sysfs_dump_register(dev, attr, addrstr, sizeof(addrstr));
  }

  return count;
}

static ssize_t anx7625_sysfs_cbldet(struct device *dev,
                                    struct device_attribute *attr, char *buf)
{
  struct i2c_client *client = to_i2c_client(dev);
  struct anx7625_data *ctx = i2c_get_clientdata(client);

  return snprintf(buf, 16, "CBL_DET=%d\n", gpiod_get_value(ctx->pdata.gpio_cbl_det));
}
...
#endif
  __ATTR(cbldet,   S_IRUGO, anx7625_sysfs_cbldet, NULL),
};
