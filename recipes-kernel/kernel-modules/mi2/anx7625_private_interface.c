...
void handle_intr_vector(void)
{
	...
	TRACE("handle_intr_vector: intr vector = 0x%x\n",  intr_vector);
	...
	TRACE("handle_intr_vector: system status = 0x%x\n",  sys_status);
	...
	if ((~INTR_MASK_SETTING) &
		...
		TRACE("handle_intr_vector: HPD_STATUS_CHANGE 0x%x\n", status & HPD_STATUS);
	}
...
void dp_hpd_change_default_func(bool on)
{
	...
	if (on == 0) {
		TRACE("dp_hpd_change_default_func: HPD low\n");
		anx7625_stop_dp_work();
	} else {
		TRACE("dp_hpd_change_default_func: HPD high\n");
		anx7625_start_dp();
	}
}
