...
void DP_Process_Start(void)
{
	...
	if ((mute_video_flag == 0) || (delay_video_cfg == 0)) {
		...
		if (slimport_edid_p == NULL) {
			slimport_edid_p = kmalloc(sizeof(struct s_edid_data), GFP_KERNEL);
		...
		if (slimport_edid_p != NULL) {
			...
			memcpy((uint8_t *)(p_edid->EDID_block_data), edid_blocks,
             (blocks_num + 1) * 128);
...
unchar sp_tx_get_edid_block(void)
{
  ...
  TRACE1("sp_tx_get_edid_block: EDID Block = %d\n", (int)(c + 1));
...
unchar edid_read(unchar offset, unchar *pblock_buf)
{
  ...
  while (cnt < 3) {
    ...
    if (c == 1) {
      ...
      for (c = 0; c < 16; c++)
        Read_Reg(RX_P0, AP_AUX_BUFF_START + c, &(pblock_buf[c]));
      return 0;
    }
  }
  return 1;
